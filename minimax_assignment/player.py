...
class PlayerControllerMinimax(PlayerController):
    ...
    def distance(self, your_hook, fish_pos, oppo_hook):
        ...
        return math.sqrt(x ** 2 + y ** 2)  # prefer to move vertically

    def strhash(self, state):
        # str: yourhookpos + oppohookpos + fishpos + fishscore
        fish_pos_dict = dict()
        ...
        key = str(state.get_hook_positions()) + str(fish_pos_dict)
        return key

    def minimax(self, node, depth, player, alpha, beta, starting_time):

        if (time.time() - starting_time > 0.055):
            raise TimeoutError
        ...
        if player == 0:
            ...
            for child in children:
                h, action = self.minimax(child, depth - 1, 1, alpha, beta, starting_time)
                if h > best_max:
                    best_max = h
                    best_action = action
            ...
            for child in children[::-1]:
                h, action = self.minimax(child, depth - 1, 0, alpha, beta, starting_time)
                if h < best_min:
                    best_min = h
                    best_action = action
        ...
        while (not timeout):
            ...
            try:
                for child in children:
                    ...
                    value, _ = self.minimax(child, depth, player, -math.inf, math.inf, starting_time)
                    scores.append(value)
                best = children[scores.index(max(scores))].move
            except:
                timeout = True
